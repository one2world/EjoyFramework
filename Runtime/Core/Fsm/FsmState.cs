...
namespace EjoyFramework.Core.Fsm
{
    ...
    public abstract class FsmState<T> where T : class
    {
        ...
        {
            // 耦合说明：此处硬转型为具体实现 Fsm<T> 以调用 internal 的 ChangeState。
            // 这绕过了 IFsm<T> 抽象；理想做法是把 ChangeState 提升为接口成员，但 IFsm<T> 接口不在本文件内，
            // 改接口签名风险较大，暂保留转型。框架内 IFsm<T> 的唯一实现即 Fsm<T>，转型在当前实现下安全。
