...
```
Runtime/
├── EjoyFramework.Core/            ← Layer 1: Engine-agnostic core (pure C#, no UnityEngine refs)
├── EjoyFramework.Core/Blobs/      ← Engine-agnostic ConfigBlob assembly (unsafe isolated)
├── EjoyFramework.Core/Ecs/        ← Self-authored pure C# ECS (separate assembly within Core)
├── EjoyFramework.Core.Unity/      ← Layer 2: Unity MonoBehaviour adapters + Helpers (core)
├── EjoyFramework.Core.Unity/Ecs/  ← ECS world lifecycle and view bindings (Core.Unity assembly)
├── EjoyFramework.Core.Jobs.Unity/ ← Unity Job System, Burst and Collections adapters
├── EjoyFramework.GamePlay/        ← Cross-genre gameplay layer (pure C#, no UnityEngine refs)
└── EjoyFramework.GamePlay.Unity/  ← Unity adapters for selected gameplay systems
Editor/
├── EjoyFramework.Core.Unity.Editor/     ← Layer 3: AssetBundle pipeline + UI editors + Config exporter (core)
└── EjoyFramework.GamePlay.Unity.Editor/ ← Editor tooling for gameplay systems
Tests/
...
The independently owned ECS worlds are documented in [Self-authored ECS](Runtime/EjoyFramework.Core/Ecs/README.md). The core has no Unity or third-party ECS dependencies; adapters belong to Core.Unity. Import the ECS Demo from Package Manager's Samples section. New modules must follow the existing [directory and naming conventions](docs/DirectoryConventions.md).
...
Total: **36 functional module directories** under `Runtime/EjoyFramework/` (17 core managers +
...
A second engine-agnostic layer (`Runtime/EjoyFramework.GamePlay/`, also `noEngineReferences:
...
systems have Unity adapters under `Runtime/EjoyFramework.GamePlay.Unity/`
...
[`Runtime/EjoyFramework.GamePlay/Units/README.md`](Runtime/EjoyFramework.GamePlay/Units/README.md).
...
|---|---|---|---|
| `EjoyFramework.Tests` (core EditMode) | `Tests/Editor/EjoyFramework.Tests/` | 549 | 536 `[Test]` + 13 `[TestCase]` |
| `EjoyFramework.GamePlay.Tests` (EditMode) | `Tests/Editor/EjoyFramework.GamePlay.Tests/` | 1253 | 1230 `[Test]` + 23 `[TestCase]` |
| `EjoyFramework.Tests.PlayMode` (PlayMode) | `Tests/Runtime/EjoyFramework.Tests.PlayMode/` | 15 | 15 `[UnityTest]` |
